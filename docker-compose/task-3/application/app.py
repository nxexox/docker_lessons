...
def send_kafka_message():
    ...
    messages = list(mongo.db.kafka.find({}))
    messages_str = '<ul>{}</ul>'.format(
        ''.join([
            '<li>Date: `{}`, Message: `{}`</li>'.format(
                doc.pop('date', ''), doc.pop('message', '')
            )
            for doc in messages
        ])
    )
    ...
    <h1>Kafka message send for worker</h1>
    <form method=post>
    ...
    <h2>Send messages: {}</h2>
    {}
    '''.format(topics, len(messages), messages_str)
